...
- **Non-distributed mode layout:** The chart layout condition MUST use `isDistributed` (checks specifically for `ComputedReportItemValueType.distributed`), NOT the `costDistribution` prop (which is truthy for any non-empty string). When "Don't distribute overhead costs" is selected, `costDistribution='total'` (truthy) but `isDistributed=false`. Using `costDistribution` as the condition would render the 5-layer distributed layout with all overhead values forced to $0.00, creating orphaned nodes with invisible zero-width links. Using `isDistributed` correctly shows the simple 3-layer Sankey (raw + markup + usage → total cost) in non-distributed mode.
