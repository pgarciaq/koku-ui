...
export const gpuMigRecommendationsBaseQuery: RosQuery = {
  limit: 10,
  offset: 0,
...
export interface GpuMigRecommendationsReportState {
...
export interface UseGpuMigRecommendationsReportProps {
  query: RosQuery;
  skipFetch?: boolean;
}

export const useGpuMigRecommendationsReport = ({
  query,
  skipFetch = false,
}: UseGpuMigRecommendationsReportProps): GpuMigRecommendationsReportState => {
  const dispatch: ThunkDispatch<RootState, any, AnyAction> = useDispatch();
  const order_by = getOrderById(query) || getOrderById(gpuMigRecommendationsBaseQuery);
  const order_how = getOrderByValue(query) || getOrderByValue(gpuMigRecommendationsBaseQuery);
  ...
  ) as unknown as GPUMIGRecommendationReport;
