...
const DataRetention: React.FC<DataRetentionProps> = ({ isDisabled }) => {
  ...
  useEffect(() => {
    if (accountSettings && !accountSettingsError) {
  ...
  }, [accountSettings, accountSettingsError]);

  if (accountSettingsError) {
    return <NotAvailable />;
  }

  // Only block the UI on the initial fetch — updates keep the current selection visible
  if (accountSettingsFetchStatus === FetchStatus.inProgress && !accountSettings) {
    return (
      <LoadingState
